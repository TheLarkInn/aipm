...
Feature: Package publishing
  As a plugin author,
  I want to pack and publish my package to a registry using the aipm-pack binary,
  so that others can discover and install it.

  Publishing is handled by the separate "aipm-pack" binary. This follows the
  principle of least privilege: consumers who only install plugins never need
  the publish binary, reducing attack surface.
  ...
  Rule: Packing creates a .aipm archive
    ...
    Scenario: Pack a plugin into a .aipm archive
      When the user runs "aipm-pack pack"
    ...
    Scenario: Pack is deterministic
      ...
      When the user runs "aipm-pack pack" twice
    ...
    Scenario: Pack dry-run shows archive contents and size
      When the user runs "aipm-pack pack --dry-run"
  ...
  Rule: Publishing uploads to the registry
    ...
    Scenario: Dry-run publish validates without uploading
      When the user runs "aipm-pack publish --dry-run"
  ...
  Rule: Consumer binary cannot publish
    ...
    Scenario: The aipm consumer binary does not have publish commands
      ...
      And the output suggests "did you mean: aipm-pack publish?"
    ...
    Scenario: The aipm consumer binary does not have pack commands
      ...
      And the output suggests "did you mean: aipm-pack pack?"
