...
| [missing-registration](rules/plugin/missing-registration.md) | error | Plugin directory is not registered in `marketplace.json` |
...
| Rule | Default | Description |
|------|---------|-------------|
| [plugin-field-mismatch](rules/marketplace/plugin-field-mismatch.md) | error | Plugin `name`/`description` in `marketplace.json` differs from `plugin.json` |
| [source-resolve](rules/marketplace/source-resolve.md) | error | Plugin `source` path in `marketplace.json` does not exist on disk |
