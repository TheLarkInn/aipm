...
- **Autocomplete** — all 19 rule IDs and severity values suggested on demand; rules with additional options (like `instructions/oversized`) also complete their per-rule fields such as `lines`, `characters`, and `resolve-imports`
