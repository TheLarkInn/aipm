...
**Research basis**:
- [research/docs/2026-04-01-port-odsp-web-lint-rules.md](../research/docs/2026-04-01-port-odsp-web-lint-rules.md) — Research on plugin validation patterns and aipm lint architecture
...
`aipm lint` ships 12 rules ([`specs/2026-03-31-aipm-lint-command.md`](./2026-03-31-aipm-lint-command.md)) focused on **skill/agent/hook content validation**: frontmatter fields, file size limits, broken path references, hook event names, and cross-tool compatibility. All 12 operate on the `.ai/` marketplace directory by scanning individual skill, agent, and hook files.
...
**Logic:** Call `scan_plugin_dirs()`. For each plugin where `plugin_json_raw.is_some()`, check if the raw content contains `"mcpServers"`. Text-level detection (not JSON key traversal) catches the key regardless of nesting depth.
