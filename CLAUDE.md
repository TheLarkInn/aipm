...
**All lock files must be compiled with the same `gh aw` version.** The repository is currently on **`v0.86.2`**. The pin lives in the `GH_AW_VERSION` env var at the top of `.github/workflows/ci.yml` (the locks also record it in their `# gh-aw-metadata:` header as `compiler_version`) — keep the two in sync. The `aw-lock-drift` job in `ci.yml` installs exactly this version, reruns `gh aw compile`, and fails the build if any lock file or `.github/aw/actions-lock.json` drifts, if compile emits untracked files, or if the locks' `compiler_version` values disagree with each other or with the pin ([#1390](https://github.com/TheLarkInn/aipm/issues/1390)). Check your local install with `gh aw version` and install the pinned release with `gh extension install github/gh-aw --pin <version>` (or `gh extension upgrade gh-aw` when intentionally moving to the newest pin) before recompiling.
