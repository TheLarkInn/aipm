...
#### `.gitattributes` and the generated maintenance workflow

Two `gh aw compile` side effects were decided deliberately in [#1392](https://github.com/TheLarkInn/aipm/issues/1392) — do not "fix" them back:

- **`.gitattributes` carries only `linguist-generated=true` for `*.lock.yml` — no `merge=ours`.** Silently resolving lock-file conflicts to the local side is exactly how locks rot out of sync with their `.md` sources, and the driver does nothing unless every contributor configures it locally. Resolve lock conflicts by recompiling with `gh aw compile`, never by merge drivers or hand-edits.
- **`.github/workflows/agentics-maintenance.yml` is generated by `gh aw compile` (v0.86.2+) and is tracked deliberately.** It runs daily (`37 0 * * *`) to close expired safe outputs and exposes manual maintenance operations via `workflow_dispatch`. Top-level `permissions: {}` with per-job least-privilege scopes and SHA-pinned actions. Do not hand-edit or delete it — to suppress generation, set `{"maintenance": false}` in `.github/workflows/aw.json` and recompile. A clean `gh aw compile` must never produce untracked files.
