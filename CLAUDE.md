...
**All lock files must be compiled with the same `gh aw` version.** The repository is currently on **`v0.86.2`**. The canonical pin is the `GH_AW_VERSION` env var at the top of `.github/workflows/ci.yml` — bump it there when upgrading, then recompile and commit every lock in the same change. Each lock file's `# gh-aw-metadata:` header also records the version it was compiled with as `compiler_version`. Check yours with `gh aw version` and upgrade with `gh extension upgrade gh-aw` before recompiling. This is enforced, not honour-system: the `aw-lock-drift` job in `ci.yml` installs the pinned version, runs `gh aw compile`, and fails the build on any resulting diff in `.github/workflows` or `.github/aw`, on untracked generated files, or on any `compiler_version` skew across the locks ([#1390](https://github.com/TheLarkInn/aipm/issues/1390)).
