...
**All lock files must be compiled with the same `gh aw` version.** The canonical pin is the `GH_AW_VERSION` env var at the top of `.github/workflows/ci.yml` (currently **`v0.86.2`**); each lock file also records the version it was built with in its `# gh-aw-metadata:` header as `compiler_version`. The `aw-lock-drift` job in `ci.yml` installs exactly `$GH_AW_VERSION`, reruns `gh aw compile`, and fails the build if any lock (or `.github/aw/actions-lock.json`) drifts, if compile emits untracked files, or if the locks disagree on `compiler_version` — so a PR that edits a `.md` without committing the regenerated lock is red. Check your local version with `gh aw version` and upgrade with `gh extension upgrade gh-aw` before recompiling.
