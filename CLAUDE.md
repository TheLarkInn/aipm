...
### `.gitattributes` and lock files

`.github/workflows/*.lock.yml` carries **`linguist-generated=true` only** — the `merge=ours` driver was deliberately dropped when the compiler rewrote the file ([#1392](https://github.com/TheLarkInn/aipm/issues/1392)). Rationale: lock files are generated artefacts, so merge conflicts in them should be loud and resolved by recompiling, not silently resolved to one side; and `merge=ours` only works when every contributor defines the driver in their local git config, making it a no-op in practice. The lock-drift CI guard ([#1390](https://github.com/TheLarkInn/aipm/issues/1390)) makes drift loud instead. Do not re-add `merge=ours` — the compiler will clobber it again on the next recompile.

### `agentics-maintenance.yml` is intentional

`.github/workflows/agentics-maintenance.yml` is **adopted, not accidental** ([#1392](https://github.com/TheLarkInn/aipm/issues/1392)). It is generated by `gh aw compile` (v0.86.2) — never hand-edit it. It runs daily at 00:37 UTC (`37 0 * * *`) to close expired safe-output discussions, issues, and PRs, and exposes manual maintenance operations via `workflow_dispatch`/`workflow_call`. Top-level `permissions: {}` with minimal per-job scopes (`discussions: write`, `issues: write`, `pull-requests: write`, etc. only on the jobs that need them). To suppress its generation, set `{"maintenance": false}` in `.github/workflows/aw.json`.
