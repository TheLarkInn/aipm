...
#### Compiler-owned files: `.gitattributes` and `agentics-maintenance.yml`

`gh aw compile` owns two files beyond the `*.lock.yml` locks. Do not hand-edit either; change them only through the compiler.

**`.gitattributes`** contains exactly one line and deliberately **no** `merge=ours` attribute:

```
.github/workflows/*.lock.yml linguist-generated=true
```

`merge=ours` was dropped when the repo moved to gh-aw `v0.86.2` ([#1392](https://github.com/TheLarkInn/aipm/issues/1392)) and the removal is intentional: on generated files it silently resolves merge conflicts to one side instead of forcing a recompile, which is precisely how locks rot out of sync with their `.md` sources. Resolve lock conflicts by rerunning `gh aw compile`, never by merging. Do not restore the attribute — the compiler rewrites the file on every compile and will clobber it again.

**`.github/workflows/agentics-maintenance.yml`** is generated by gh-aw's maintenance-workflow generator and is **adopted**, not suppressed. It runs daily (`cron: "37 0 * * *"`) to close expired safe-output issues/PRs/discussions and clean stale cache-memory entries — all no-ops today since no `.md` workflow uses `expires:` — and exposes maintainer-gated manual operations (`workflow_dispatch`: `update`, `upgrade`, `validate`, `forecast`, `activity_report`, …) with per-job minimal `GITHUB_TOKEN` scopes (no PAT). Keeping the compiler default means expiring safe outputs work out of the box if a workflow ever adopts them. To suppress it, set `{"maintenance": false}` in `.github/workflows/aw.json` — do not delete the file by hand, or the next compile recreates it as an untracked file.
