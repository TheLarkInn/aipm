...
This regex must stay identical to `COV_IGNORE_REGEX` in `.github/workflows/ci.yml` and to the pattern used in `.github/workflows/improve-coverage.md`. If they diverge, local runs, CI, and the coverage agent all measure different code against the same 89% gate.
...
`agentics-maintenance.yml` is also generated by `gh aw compile`. It is gh-aw's companion maintenance workflow: daily it closes *expired* agentic issues/PRs/discussions and cleans cache memories, and it exposes on-demand operations via `workflow_dispatch` (`validate`, `create_labels`, `activity_report`, `forecast`, `update_pull_request_branches`, `safe_outputs` replay). Its cleanup only touches entities whose body carries **both** a gh-aw marker (`gh-aw-workflow-id:` / `gh-aw-agentic-workflow:`) **and** a checked expiration marker, so hand-filed issues are never affected.
...
```bash
gh extension install github/gh-aw --pin v0.85.4   # version pinned by CI
gh aw compile <name>                              # e.g. gh aw compile improve-coverage
```

Use the version pinned in `GH_AW_VERSION` in `.github/workflows/ci.yml` — that is the single source of truth. The `aw-lock-drift` CI job installs that exact version, recompiles, and fails the build if the committed locks differ, if compilation produces untracked files, or if the locks disagree on `compiler_version`.

Commit both the `.md` source and the regenerated `.lock.yml` together. The compiled lock file is the canonical version GitHub Actions runs. Never hand-edit a `.lock.yml`.

Bumping `GH_AW_VERSION` is a deliberate change: update it, recompile everything, and commit the locks in the same PR.

### Operational gotchas

Hard-won lessons; each of these caused a real, long-lived outage.

- **Bot merges emit no `push` event.** GitHub suppresses workflow events triggered via `GITHUB_TOKEN` to prevent recursion. Anything relying on `on: push: [main]` will silently never run when PRs are auto-merged by a bot. This disabled post-merge `CI` and `Update Docs` for roughly three months.
- **A workflow that emits only `noop` is reported as `success`.** A livelocked agent looks perfectly healthy on the Actions dashboard. Check what a run *did*, not just its conclusion. Coverage Improver spent weeks no-op'ing on a conflicting PR that could never merge.
- **External pins rot.** An AWF firewall release referenced by a lock file was deleted upstream, failing `reverse-binary-analysis` for 10+ consecutive weeks with no alert. Version skew between locks is what let one bad pin survive unnoticed.
- **`add-labels` needs a triggering issue or PR.** On a `schedule` run there is none, and the safe output fails with `No issue/PR number available`. Put `labels:` on `create-issue` instead.
- **Keep the coverage ignore-regex identical** in `ci.yml`, `improve-coverage.md`, and this file. When they diverge, the agent writing coverage tests measures a different denominator from the gate that judges them.
