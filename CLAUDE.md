...
**All lock files must be compiled with the same `gh aw` version.** The repository is currently on **`v0.86.2`**. The pin is the `GH_AW_VERSION` env var in `.github/workflows/ci.yml`, and the `aw-lock-drift` CI job enforces it: it recompiles with the pinned version and fails the build if any `.lock.yml` differs, if `gh aw compile` emits untracked files, or if the locks' `# gh-aw-metadata:` `compiler_version` values disagree with the pin. Check your local version with `gh aw version` and upgrade with `gh extension upgrade gh-aw` before recompiling. When bumping the pin, update `GH_AW_VERSION` and this section in the same commit as the recompiled locks.
