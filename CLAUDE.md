...
**All lock files must be compiled with the same `gh aw` version.** The repository is currently on **`v0.86.2`**. The canonical pin is the `GH_AW_VERSION` env var at the top of `.github/workflows/ci.yml` — bump it there when upgrading. Every lock file's `# gh-aw-metadata:` header records the version it was compiled with as `compiler_version`. Check yours with `gh aw version` and upgrade with `gh extension upgrade gh-aw` before recompiling. The `aw-lock-drift` job in `ci.yml` installs the pinned version, runs `gh aw compile`, and fails the build on any drift in `.github/workflows` or `.github/aw` — including untracked generated files and compiler-version mismatches — so out-of-date locks cannot slip through on the honour system ([#1390](https://github.com/TheLarkInn/aipm/issues/1390)).
