...
mod tests {
    ...
    /// Exercises `MockFs`'s unused `Fs` trait stub methods
    /// (`create_dir_all`, `write_file`, `read_to_string`, `read_dir`), which
    /// no other test in this module calls since `find_marketplace` only
    /// needs `exists`.
    #[test]
    fn mock_fs_stub_methods_are_covered() {
        let fs = MockFs::new();
