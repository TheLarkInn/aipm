...
mod tests {
    ...
    #[test]
    fn mock_fs_stub_methods_are_exercised() {
        // Covers the `MockFs` trait-method bodies that `find_marketplace`
        // itself never calls (it only calls `exists`), so they otherwise
        // remain unreached branches in coverage.
        let fs = MockFs::new();
