...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        // Create a *directory* named "aipm.toml" so `exists()` returns true
        // but `read_to_string` fails with "Is a directory" — this covers the
        // `Err(e)` read-error branch (distinct from the TOML-parse-error
