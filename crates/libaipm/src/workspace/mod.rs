...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        ...
        // returns true but `read_to_string` fails — this exercises the
        // `Err(e)` branch (and its `tracing::debug!` logging) inside
        // `find_workspace_root`, distinct from the invalid-TOML branch
