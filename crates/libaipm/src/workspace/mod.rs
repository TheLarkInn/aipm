...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        // If a path named "aipm.toml" exists as a directory rather than a file,
        // `fs.exists` returns true but `fs.read_to_string` fails — this covers the
        // `Err(e)` branch in `find_workspace_root` (the read-error log-and-continue
        // path), distinct from the sibling TOML-parse-error branch already covered
