...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        ...
        // regular file, `fs.exists()` returns true but
        // `std::fs::read_to_string` fails with "Is a directory" — this
        // covers the `Err(e)` branch (read error, distinct from the parse
        // error branch) in `find_workspace_root`.
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
