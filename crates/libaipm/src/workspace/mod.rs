...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        ...
        // `exists()` returns true but `read_to_string` fails with an IO error —
        // this covers the `Err(e)` branch in `find_workspace_root`'s read match,
        // distinct from the TOML-parse-error branch covered above.
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
