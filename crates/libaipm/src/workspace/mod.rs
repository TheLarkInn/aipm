...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        // If "aipm.toml" exists but is a directory rather than a file,
        // `fs.exists()` returns true while `fs.read_to_string()` fails.
        // This exercises the `Err(e)` branch in `find_workspace_root` that
        // logs and continues walking up instead of parsing the manifest.
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
