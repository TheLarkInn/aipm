...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        ...
        // fs.exists() returns true but fs.read_to_string() fails with an
        // I/O error ("Is a directory"). This exercises the `Err(e)` branch
        // in find_workspace_root's outer match (the "could not read
        // manifest" debug-log path), distinct from the TOML-parse-error
