...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        // Create a directory named `aipm.toml` so `fs.exists()` reports true
        // (the path exists) but `fs.read_to_string()` fails with an I/O error
        // (it's a directory, not a file) — covers the "could not read
        // manifest during workspace discovery" branch.
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
