...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        // Make `aipm.toml` a directory instead of a file so `read_to_string`
        // fails with an I/O error, exercising the "could not read manifest"
        // branch in `find_workspace_root`.
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
