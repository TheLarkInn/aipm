...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        ...
        // file, `fs.exists()` returns true but `read_to_string` errors out —
        // this exercises the read-error branch (as opposed to the
        // parse-error branch covered by `find_root_skips_invalid_toml`).
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
