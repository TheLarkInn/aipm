...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        // If a path named "aipm.toml" exists as a directory rather than a file,
        // std::fs::read_to_string fails with an I/O error — this covers the
        // `Err(e)` read-error branch in `find_workspace_root` (as opposed to
        // the TOML-parse-error branch already covered by
        // `find_root_skips_invalid_toml`).
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();

        // Create a *directory* called "aipm.toml" at the workspace root so
        // `exists()` returns true but `read_to_string` errors.
