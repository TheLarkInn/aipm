...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        // `aipm.toml` is a directory rather than a regular file, so
        // `Fs::exists` returns `true` but `Fs::read_to_string` fails with an
        // I/O error — exercising the `Err(e)` branch of the outer match in
        // `find_workspace_root` (distinct from the inner TOML-parse-error
