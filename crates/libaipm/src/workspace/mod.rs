...
mod tests {
    ...
    fn find_root_skips_unparseable_manifest_and_continues_up() {
        ...
        // An intermediate directory has an unparseable `aipm.toml` — this
        // exercises the `Err(e)` branch of `toml::from_str` in
        // `find_workspace_root`, which logs and continues walking upward
    ...
    fn find_root_skips_unreadable_manifest_and_continues_up() {
        ...
        // directory (not a file), so `fs.exists()` reports true but
        // `read_to_string` fails. This exercises the `Err(e)` branch of
        // `fs.read_to_string` in `find_workspace_root`, which logs and
        // continues walking upward instead of erroring out.
        let subdir = root.join("sub");
