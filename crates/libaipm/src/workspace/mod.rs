...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        ...
        // regular file, `exists()` returns true but `read_to_string` fails
        // with an I/O error — this covers the "could not read manifest"
        // branch in `find_workspace_root` (as opposed to the "unparseable
        // manifest" branch already covered by `find_root_skips_invalid_toml`).
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
