...
mod tests {
    ...
    fn find_root_skips_unreadable_manifest() {
        // `aipm.toml` exists but is itself a directory, so `fs.exists()`
        // returns true while `fs.read_to_string()` fails — exercising the
        // `Err(e)` branch of the outer read match (distinct from the TOML
        // parse-error branch covered above).
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
