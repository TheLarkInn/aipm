...
mod tests {
    ...
    fn ci_azure_warnings_only_emits_succeeded_with_issues() {
        // Covers the true arm of `error_count == 0 && warning_count > 0`
        // (line 376): when every diagnostic is a warning, Azure DevOps
        // should be told the run "succeeded with issues" rather than
        // failing outright.
        let outcome = Outcome {
            diagnostics: vec![Diagnostic {
                rule_id: "skill/missing-description".to_string(),
