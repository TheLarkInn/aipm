...
mod tests {
    ...
    #[test]
    fn ci_azure_succeeded_with_issues_when_only_warnings() {
        // Covers the True branch of
        // `outcome.error_count == 0 && outcome.warning_count > 0` (line 372):
        // when only warnings (no errors) are present, the reporter emits the
        // `##vso[task.complete result=SucceededWithIssues;]` command so the
        // Azure DevOps pipeline surfaces the issues without failing the build.
        let outcome = Outcome {
            diagnostics: vec![Diagnostic {
                rule_id: "skill/missing-description".to_string(),
                severity: Severity::Warning,
        ...
        assert!(
            ...
            "expected SucceededWithIssues command when only warnings present: {output}"
        );
    }
