...
mod tests {
    ...
    /// `create_dir_all` on the store path succeeds (it's a real directory),
    /// but `File::create(&lock_path)` then fails with EISDIR, exercising the
    /// `Error::Io` mapping around the lock-file creation call.
    #[cfg(unix)]
    #[test]
    fn lock_errors_when_lock_file_path_is_a_directory() {
        ...
        std::fs::create_dir_all(&store_path).unwrap();
        // Occupy the `.lock` path with a directory instead of a file.
