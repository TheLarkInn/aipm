...
mod tests {
    ...
    /// When the target's parent directory cannot be created (because a
    /// regular file already occupies that path), `link_to` should propagate
    /// the `create_dir_all` error instead of attempting the hard link.
    #[cfg(unix)]
    #[test]
    fn link_to_errors_when_parent_creation_fails() {
        ...
        assert!(result.is_err(), "expected link_to to fail when parent creation fails");
    }
