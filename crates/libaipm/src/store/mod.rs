...
mod tests {
    ...
    /// If a directory already exists at the `.lock` path (instead of a
    /// regular file), `std::fs::File::create` fails with an I/O error,
    /// exercising the `File::create` error-mapping branch in `lock()`.
    #[test]
