...
mod tests {
    ...
    fn open_fails_when_path_is_a_directory() {
        // Opening a directory with `.write(true)` fails with `IsADirectory`,
        // exercising the `OpenOptions::open` error branch in `LockedFile::open`
        // (parent creation succeeds since the directory already exists).
        let temp = tempfile::tempdir().unwrap_or_else(|_| unreachable_tempdir());
