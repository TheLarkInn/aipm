...
mod tests {
    ...
    fn read_content_fails_on_invalid_utf8() {
        // Write raw non-UTF-8 bytes directly (bypassing `write_content`, which
        // only accepts `&str`), then confirm `read_content` surfaces the
        // `Error::Read` branch when `read_to_string` rejects invalid UTF-8.
        let temp = tempfile::tempdir().unwrap_or_else(|_| unreachable_tempdir());
