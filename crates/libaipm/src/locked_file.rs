...
mod tests {
    ...
    fn open_fails_when_path_is_a_directory() {
        // The parent directory creation succeeds (it already exists), but
        // opening the path itself for read/write fails because it is a
        // directory, not a regular file. This exercises the `.open(path)`
        // `map_err` branch distinct from the parent `create_dir_all` error
