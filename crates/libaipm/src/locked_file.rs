...
mod tests {
    ...
    fn open_fails_when_path_is_a_directory() {
        // The parent directory exists and is created successfully, but the
        // target path itself is a directory, so `OpenOptions::open` fails
        // with `IsADirectory` — exercising the file-open error branch in
        // `LockedFile::open` (distinct from the earlier mkdir-error test).
        let temp = tempfile::tempdir().unwrap_or_else(|_| unreachable_tempdir());
