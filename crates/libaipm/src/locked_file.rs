...
mod tests {
    ...
    fn read_content_fails_on_invalid_utf8() {
        // Write invalid UTF-8 bytes directly to the file, bypassing
        // `write_content` (which only accepts valid `&str`). This exercises
        // the `Error::Read` branch in `read_content` when
        // `read_to_string` encounters bytes that are not valid UTF-8.
        let temp = tempfile::tempdir().unwrap_or_else(|_| unreachable_tempdir());
