...
mod tests {
    ...
    #[test]
    fn read_content_rejects_invalid_utf8() {
        // Write invalid UTF-8 bytes directly (bypassing `LockedFile` so the
        // write isn't itself subject to the lock). `read_to_string` then
        // fails with `InvalidData`, exercising the `Error::Read` mapping
        // branch in `read_content` without needing to force an OS-level I/O
        // failure.
        let temp = tempfile::tempdir().unwrap_or_else(|_| unreachable_tempdir());
