...
mod tests {
    ...
    fn open_fails_when_path_itself_is_a_directory() {
        // The parent directory already exists, so `create_dir_all` is a
        // no-op success and the `if let Some(parent)` branch is taken
        // without error. `OpenOptions::open` then fails because `path` is a
        // directory, not a regular file — exercising the file-open error
        // branch (distinct from the mkdir error branch above).
        let temp = tempfile::tempdir().unwrap_or_else(|_| unreachable_tempdir());
        let dir_path = temp.path().join("i_am_a_directory");
