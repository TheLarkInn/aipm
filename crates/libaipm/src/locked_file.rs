...
mod tests {
    ...
    fn read_content_fails_on_invalid_utf8() {
        // Write bytes that are not valid UTF-8 directly to the file, bypassing
        // `LockedFile::write_content`, so `read_to_string` fails and the
        // `Error::Read` branch in `read_content` is exercised.
        let temp = tempfile::tempdir().unwrap_or_else(|_| unreachable_tempdir());
