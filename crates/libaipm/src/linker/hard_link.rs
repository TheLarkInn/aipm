...
mod tests {
    ...
    #[test]
    fn assemble_remove_existing_target_file_errors() {
        // If `target_dir` exists but is a regular file (not a directory),
        // `remove_dir_all` fails with `NotADirectory`, covering the error
        // mapping on the initial cleanup call.
        let tmp = tempfile::tempdir().expect("tempdir");
        let store = store::Store::new(tmp.path().join("store"));
        let hash = store.store_file(b"content").expect("store file");

        let target = tmp.path().join("links").join("file-not-dir-pkg");
