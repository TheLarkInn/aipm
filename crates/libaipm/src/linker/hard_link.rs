...
mod tests {
    ...
    #[test]
    fn assemble_returns_error_when_existing_target_is_not_a_directory() {
        // If `target_dir` already exists but is a regular file (not a
        // directory), `std::fs::remove_dir_all` fails with `NotADirectory`,
        // exercising the `map_err` branch on the initial cleanup step.
        let (tmp, store, file_hashes) = make_store_and_package();
        let target = tmp.path().join("links").join("not-a-dir");

        std::fs::create_dir_all(target.parent().expect("parent")).expect("create parent");
        std::fs::write(&target, b"i am a file, not a directory").expect("write file");
