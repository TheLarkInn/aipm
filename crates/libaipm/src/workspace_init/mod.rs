...
mod tests {
  ...
  "enabledPlugins": {
    ...
    /// Existing `aipm.toml` omits `[workspace].engines` entirely (disk_engines
    /// = `None`), but the wizard answer selected Claude (`Some(w)`). Covers
    /// the `(Some(w), None) => { ... }` match arm in `compare_and_warn` — a
    /// distinct branch from the `(Some(w), Some(d)) if w != d` conflict arm
    /// exercised by `init_existing_aipm_toml_warns_on_engine_conflict`.
    #[test]
    #[tracing_test::traced_test]
    fn init_existing_aipm_toml_no_disk_engines_warns_wizard_only() {
        let (tmp, _guard) = make_temp_dir("existing-toml-no-disk-engines");
        // Omit `engines` entirely so `[workspace]` has no `engines` field.
        let manifest = crate::manifest::builder::build_workspace_manifest(
            &crate::manifest::builder::WorkspaceManifestOpts {
                members: &[".ai/*".to_string()],
                plugins_dir: Some(".ai"),
        ...
        let opts = Options {
            ...
            marketplace_name: "local-repo-plugins",
            engines_scaffold: libaipm_engine_spec::EngineSet::CLAUDE,
            // Wizard picked Claude, but on-disk manifest declares no engines.
            engines_support: Some(libaipm_engine_spec::EngineSet::CLAUDE),
        };
        let result = init(&opts, &adaptors, &crate::fs::Real);
        assert!(result.is_ok(), "idempotent init must succeed: {result:?}");
        ...
        assert!(
            ...
            "expected wizard-only-engines warn event"
        );
