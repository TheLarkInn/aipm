...
mod tests {
    ...
    /// Covers the `!any_created && !any_found` case (line 208's `any_found`
    /// False arm): with both `workspace` and `marketplace` disabled, `init`
    /// produces zero actions, so neither the "created" nor the "found
    /// existing" tail-warn condition is reached — the function should
    /// simply return an empty action list without emitting the "found
