...
mod tests {
    ...
    #[test]
    fn init_with_both_phases_disabled_skips_nothing_to_do_warning() {
        // #850 Q9.5: the tail "nothing to do" warning is only emitted when
        // `any_found` is true (i.e. init actually found pre-existing
        // artifacts it skipped). With both `workspace` and `marketplace`
        // disabled, `actions` stays empty, so `any_created` and `any_found`
        // are both `false` — exercising the previously-uncovered False side
        // of `any_found` in the `!any_created && any_found` guard.
        ...
        let result = init(&opts, &adaptors, &crate::fs::Real);
        assert!(result.is_ok(), "no-op init must still succeed: {result:?}");
