...
mod tests {
    ...
    /// Covers the `False` side of `if !any_created && any_found` (#850 Q9.5):
    /// when neither `workspace` nor `marketplace` is requested, `init` pushes
    /// no actions at all, so both `any_created` and `any_found` are `false`
    /// and the "nothing to do" warning must NOT fire.
    #[test]
    fn init_with_no_phases_requested_produces_no_actions() {
        ...
        assert!(actions.is_empty(), "no phases requested → no actions expected: {actions:?}");
