...
mod tests {
    ...
    #[test]
    fn init_with_both_flags_disabled_produces_no_actions_and_no_warn() {
        // Neither `workspace` nor `marketplace` is requested, so `init` does
        // nothing at all: `actions` stays empty, `any_created` and
        // `any_found` are both `false`, and the `!any_created && any_found`
        // tail-warn guard evaluates its `any_found` operand (rather than
        // short-circuiting on a `true` `!any_created`) and correctly stays
        // silent. This is distinct from the idempotent-rerun tests, which
        // request a phase that already exists (`any_found = true`).
        ...
        let actions = result.ok().map(|r| r.actions).unwrap_or_default();
        assert!(actions.is_empty(), "no phase was requested, so no actions should be recorded");
