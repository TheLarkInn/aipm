...
mod tests {
  ...
  "enabledPlugins": {
    ...
    /// Existing `aipm.toml` declares no `engines` field at all, but the
    /// wizard answer selects an engine set. Covers the `(Some(w), None)`
    /// arm of `compare_and_warn`'s match — distinct from the `w != d`
    /// mismatch case above, since here the disk side is entirely absent.
    #[test]
    #[tracing_test::traced_test]
    fn init_existing_aipm_toml_wizard_engines_but_disk_has_none_warns() {
        ...
        assert!(
            logs_contain("on-disk manifest does not declare them"),
            "expected warn event for wizard-selected engines missing on disk"
        );
