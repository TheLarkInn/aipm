...
mod tests {
    ...
    #[test]
    fn init_with_all_flags_disabled_produces_no_actions_and_no_warn() {
        // Covers the `any_found` false side of `!any_created && any_found`
        // in `init`'s tail warning check: when neither `workspace` nor
        // `marketplace` is requested, `actions` stays empty so both
        // `any_created` and `any_found` are false, and the warning must
        // be skipped (this is a deliberate no-op run, not a "found
        // nothing to do" run).
        let (tmp, _guard) = make_temp_dir("ws-all-flags-disabled");
