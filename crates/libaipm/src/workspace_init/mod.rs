...
mod tests {
    ...
    #[test]
    fn init_no_flags_skips_tail_warning_branch() {
        // Exercise the `any_found` false arm of `if !any_created && any_found`
        // (line 208): with both `workspace` and `marketplace` disabled, no
        // actions are recorded at all, so `any_found` is `false` and the
        // tail "nothing to do" warning is skipped.
