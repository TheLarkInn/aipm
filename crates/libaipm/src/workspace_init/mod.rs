...
mod tests {
    ...
    fn init_with_both_phases_disabled_produces_no_actions_and_no_warning() {
        // Covers the False side of `any_found` in the tail idempotency-warning
        // check: `if !any_created && any_found`. When both `workspace` and
        // `marketplace` are false, `init` never pushes any `InitAction`, so
        // both `any_created` and `any_found` are false and the warning is
        // skipped entirely — the "nothing requested" case, distinct from the
        // "requested but already existed" case exercised elsewhere.
        ...
        assert!(result.is_ok_and(|r| r.actions.is_empty()), "expected zero actions");
