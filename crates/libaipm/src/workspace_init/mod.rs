...
mod tests {
    ...
    /// Covers the `!any_created && any_found` False branch of `init` (line
    /// 208): when neither `--workspace` nor `--marketplace` is requested,
    /// `actions` stays empty, so both `any_created` and `any_found` are
    /// `false` and the "nothing to do" warning must not fire.
    #[test]
    fn init_with_no_phases_requested_produces_no_actions() {
        ...
        assert!(
            actions.is_empty(),
            "no phase was requested, so no actions should be produced: {actions:?}"
        );
