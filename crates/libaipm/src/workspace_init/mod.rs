...
mod tests {
    ...
    /// Covers the False branch of `any_found` in the tail-warning check
    /// (`!any_created && any_found`, line 208): when neither `--workspace`
    /// nor `--marketplace` is requested, `actions` stays empty, so both
    /// `any_created` and `any_found` are `false` and the "nothing to do"
    /// warning must be skipped.
    #[test]
