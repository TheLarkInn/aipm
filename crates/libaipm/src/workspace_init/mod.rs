...
mod tests {
    ...
    #[test]
    fn init_with_no_flags_produces_no_actions_and_no_warn() {
        // Covers the False branch of `any_found` in the tail-warn check
        // (`if !any_created && any_found`): with both `workspace` and
        // `marketplace` disabled, `init` performs no phases, so `actions`
        // is empty and both `any_created` and `any_found` are false. This
        // is a deliberate no-op run (nothing requested), distinct from a
        // "found existing, nothing created" run, so no warning should fire
