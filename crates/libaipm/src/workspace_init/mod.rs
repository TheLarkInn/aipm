...
mod tests {
    ...
    fn init_with_both_flags_disabled_produces_no_actions() {
        // Both `workspace` and `marketplace` are false, so `init` never pushes
        // any `InitAction`. This exercises the `any_found` False arm of
        // `if !any_created && any_found` (line 208): `any_created` is also
        // false, but the tail warn must not fire because nothing was
        // requested in the first place (`any_found` is false too).
        ...
        let actions = result.ok().map(|r| r.actions).unwrap_or_default();
        assert!(actions.is_empty(), "no actions should be produced when nothing is requested");
