...
mod tests {
    ...
    #[test]
    fn init_with_no_phases_produces_no_actions_and_no_warn() {
        // Covers the `!any_created && any_found` False branch (via `any_found`
        // being false) at the tail of `init`: with both `workspace` and
        // `marketplace` disabled, `actions` stays empty, so both `any_created`
        // and `any_found` are false and the "found nothing to do" warn must
        // not fire (there is nothing to assert on the warn directly, but the
        // call must still succeed and return an empty action list).
