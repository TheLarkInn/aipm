...
mod tests {
    ...
    #[test]
    fn init_neither_workspace_nor_marketplace_emits_no_warning() {
        // Covers the False branch of `!any_created && any_found` in `init`:
        // when both flags are disabled, no actions are produced at all, so
        ...
        let result = init(&opts, &adaptors, &crate::fs::Real);
        assert!(result.is_ok_and(|r| r.actions.is_empty()), "expected no actions to be produced");
