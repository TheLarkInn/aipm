...
mod tests {
    ...
    /// (line 208): when both `--workspace` and `--marketplace` are disabled,
    /// `init` performs no phases at all, so `actions` stays empty and
    /// neither `any_created` nor `any_found` is ever true. This is distinct
    /// from the #850 "found nothing to do" warning, which only fires when
    /// the user explicitly requested a phase that turned out to already
    /// exist.
    #[test]
    fn init_with_no_phases_requested_produces_no_actions() {
        ...
        let result = init(&opts, &adaptors, &crate::fs::Real);
        assert!(result.is_ok());
        assert!(result.is_ok_and(|r| r.actions.is_empty()), "no phases requested → no actions");
