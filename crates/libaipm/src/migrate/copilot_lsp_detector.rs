...
mod tests {
    ...
    #[test]
    fn detect_lsp_json_directly_at_source_dir() {
        // lsp.json exists directly under source_dir (not via the .github
        // fallback), exercising the `fs.exists(&direct_path)` == true branch.
        let mut fs = MockFs::new();
        fs.exists.insert(PathBuf::from("/project/.copilot/lsp.json"));
        fs.files.insert(
            PathBuf::from("/project/.copilot/lsp.json"),
            r#"{"go-lsp":{"command":"gopls"}}"#.to_string(),
        );
