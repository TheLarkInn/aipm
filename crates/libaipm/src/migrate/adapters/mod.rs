...
mod tests {
    ...
    /// Covers the `Err(Error::UnsupportedSource(..))` branch of
    /// `StubAdapter::to_artifact`, which no other test in this module
    /// exercises (`stub_adapter_does_not_apply` only calls `applies_to`).
    #[test]
