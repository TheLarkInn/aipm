...
mod tests {
    ...
    fn stub_adapter_to_artifact_returns_unsupported_source_err() {
        // Exercises the `to_artifact` body on `StubAdapter`, which is never
        // invoked by the runtime registry (its `applies_to` always returns
        // `false`), so it was otherwise dead in coverage terms.
        let stub = StubAdapter;
        let feat = DiscoveredFeature {
            kind: crate::discovery::FeatureKind::Skill,
            source: crate::discovery::types::DiscoverySource::CLAUDE,
