...
mod tests {
    ...
    fn stub_adapter_to_artifact_returns_unsupported_source_error() {
        // Exercises the StubAdapter::to_artifact error path, which is never
        // reached by the other tests above (they only call `applies_to` and
        // `name`).
        struct NoopFs;
        impl Fs for NoopFs {
        ...
        assert!(err.contains("stub never produces artifacts"), "unexpected error: {err}");
    }
}
