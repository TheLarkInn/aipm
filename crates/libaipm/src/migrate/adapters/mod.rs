...
mod tests {
    ...
    /// Minimal no-op [`Fs`] used only to satisfy `to_artifact`'s signature;
    /// `StubAdapter::to_artifact` never touches the filesystem.
    struct NoopFs;
    impl Fs for NoopFs {
    ...
    fn stub_adapter_to_artifact_returns_unsupported_source_error() {
        // Covers the `StubAdapter::to_artifact` error branch, which is
        // otherwise never exercised because `applies_to` always returns
        // `false` for the stub.
        let stub = StubAdapter;
        let feat = DiscoveredFeature {
            kind: crate::discovery::FeatureKind::Skill,
