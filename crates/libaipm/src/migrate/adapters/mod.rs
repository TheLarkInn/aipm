...
mod tests {
    ...
    #[test]
    fn stub_adapter_to_artifact_always_errors() {
        // Exercises `StubAdapter::to_artifact`, which is otherwise never
        // invoked since `applies_to` always returns `false` in production.
        let stub = StubAdapter;
        let feat = DiscoveredFeature {
            kind: crate::discovery::FeatureKind::Skill,
            source: crate::discovery::types::DiscoverySource::CLAUDE,
        ...
        assert!(
            matches!(result, Err(Error::UnsupportedSource(reason)) if reason == "stub never produces artifacts")
        );
    }
}
