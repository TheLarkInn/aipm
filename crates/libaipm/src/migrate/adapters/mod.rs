...
mod tests {
    ...
    #[test]
    fn stub_to_artifact_returns_unsupported_source_err() {
        // Exercises the `to_artifact` error branch on `StubAdapter`, which is
        // otherwise never invoked since `applies_to` always returns `false`.
        let stub = StubAdapter;
        let feat = DiscoveredFeature {
            kind: crate::discovery::FeatureKind::Skill,
            source: crate::discovery::types::DiscoverySource::CLAUDE,
