...
mod tests {
    ...
    fn stub_adapter_to_artifact_returns_unsupported_source_error() {
        // Covers StubAdapter::to_artifact, which is never exercised by the
        // other tests above (they only call `name` and `applies_to`).
        let stub = StubAdapter;
        let feat = DiscoveredFeature {
            kind: crate::discovery::FeatureKind::Skill,
            source: crate::discovery::types::DiscoverySource::CLAUDE,
        ...
        let err = stub.to_artifact(&feat, &fs).unwrap_err();
        assert!(
            matches!(err, Error::UnsupportedSource(msg) if msg.contains("stub never produces artifacts"))
        );
    }
}
