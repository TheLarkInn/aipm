//! Copilot skill adapter: turns a [`DiscoveredFeature`] of kind
//! `Skill`/engine `Copilot` into an [`Artifact`].
//!
//! Replaces the filesystem-walking body of
//! [`crate::migrate::copilot_skill_detector::CopilotSkillDetector`] with a
//! per-feature transformation. The discovery layer (`discovery::discover`)
//! is now responsible for finding the SKILL.md files; this adapter only
//! parses one at a time.
