...
exists. If it does, **verify the PR can actually make progress before anything
else**: a conflicting, failing, stuck, or stale PR must be repaired or
superseded — never silently waited on. If the existing PR is healthy, inspect
...
A run that only calls `noop` is classified as success. That is how a past
livelock — a conflicting PR parking this workflow for weeks while every run
exited green — stayed invisible. Step 8 therefore escalates repeated identical
`noop`s on the same PR to a visible issue instead of letting silence pass for
success.
...
- **The PR's most recent commit is more than 24 hours old and it is still
  unmerged** (checks failing, absent, or green for a day without a merge) →
  stale. Go to **Step 5** (close as superseded and start fresh). Do **not**
  `noop`.
...
1. Check out the PR branch locally (the checkout fetches every branch) and
   ...
   Prefer merging over rebasing — a merge is a plain push, while a rebase
