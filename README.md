...
Migrate existing `.claude/` or `.github/` configurations into marketplace plugins. Detects skills, agents, MCP servers, hooks, commands, and output styles from both Claude Code and Copilot CLI workspaces.
...
| `--source <SRC>` | Source folder to scan (e.g., `.claude`, `.github`). Omit to discover recursively |
...
| Source | Artifacts |
|--------|-----------|
| Claude Code (`.claude/`) | Skills (`SKILL.md`), agents (`agents/*.md`), MCP servers (`.mcp.json`), hooks (`hooks.json`), commands (`commands/*.md`), output styles |
| Copilot CLI (`.github/`) | Skills (`skills/<name>/`), agents (`agents/<name>.md` or `<name>.agent.md`), MCP servers (`.copilot/mcp-config.json`), hooks (`hooks.json`), extensions (`extensions/<name>/`), LSP servers (`lsp.json`) |
